<?php
// /catai/api/get_news_providers_safe.php
...
try {
  ...
  // Obtener todos los proveedores de noticias habilitados (igual que get_data_providers_safe.php)
  $stmt = $pdo->prepare('
  ...
  // Transformar para el frontend (igual que get_data_providers_safe.php)
  $formattedProviders = [];
  foreach ($providers as $provider) {
    $formattedProviders[] = [
      ...
      'description' => $provider['name'] . ' - ' . ucfirst($provider['category'])
    ];
  }
  ...
  json_out([
    ...
    error_log("Error en get_news_providers_safe.php: " . $e->getMessage());
