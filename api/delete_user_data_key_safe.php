...
try {
    ...
    if ($recordId <= 0) {
        json_out(['error' => 'invalid-id', 'message' => 'ID de registro inválido'], 400);
    }

    $pdo = db();
    
    // BORRAR DIRECTAMENTE por ID del registro en user_data_api_keys
    $stmt = $pdo->prepare('DELETE FROM user_data_api_keys WHERE id = ? AND user_id = ?');
    ...
    error_log("Error en delete_user_data_key_safe.php: " . $e->getMessage());
