...
try {
    ...
    // BORRAR DIRECTAMENTE por ID del registro en user_trade_api_keys
    $stmt = $pdo->prepare('DELETE FROM user_trade_api_keys WHERE id = ? AND user_id = ?');
    ...
    error_log("Error en delete_user_trade_key_safe.php: " . $e->getMessage());
