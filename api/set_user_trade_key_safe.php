...
try {
    ...
    $stmt = $pdo->prepare('SELECT id, slug, name FROM trade_providers WHERE id = ? AND is_enabled = 1');
    $stmt->execute([$providerId]);
    $provider = $stmt->fetch(PDO::FETCH_ASSOC);
    
    if (!$provider) {
        json_out(['error' => 'provider-not-found', 'message' => 'Proveedor de trading no encontrado o deshabilitado'], 404);
    }
    ...
    $stmt = $pdo->prepare('INSERT INTO user_trade_api_keys
    ...
    error_log("Error en set_user_trade_key_safe.php: " . $e->getMessage());
