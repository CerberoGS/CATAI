...
try {
  ...
  $baseDir = dirname(__DIR__) . '/static/uploads/' . $userId;
  if (!is_dir($baseDir)) @mkdir($baseDir, 0775, true);

  // security: ensure .htaccess to disable PHP
  $hta = dirname(__DIR__) . '/static/uploads/.htaccess';
  if (!is_file($hta)) {
  ...
  $url = '/bolsa/static/uploads/' . $userId . '/' . $name;
