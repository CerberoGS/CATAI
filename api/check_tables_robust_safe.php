...
try {
    ...
    $results = [
        ...
        'message' => 'Verificación robusta de tablas completada',
        'tables_status' => []
    ];
    ...
    foreach ($critical_tables as $table) {
        try {
            // Verificar si la tabla existe usando una consulta más simple
            ...
            if ($table_exists) {
                ...
                try {
                    ...
                    try {
                        $stmt = $pdo->query("SELECT COUNT(*) as count FROM `$table` WHERE created_by = $user_id");
                        $user_count = $stmt->fetch(PDO::FETCH_ASSOC)['count'];
                    } catch (Exception $e2) {
