...
try {
    ...
    $stmt = $pdo->prepare('
        SELECT 
            ...
            dak.error_count,
            ap.slug,
            ap.name as provider_label,
            ap.url_request
        FROM user_ai_api_keys dak
        JOIN ai_providers ap ON dak.provider_id = ap.id
    ...
    if ($responseData) {
        switch (strtolower($keyData['slug'])) {
            case 'openai':
                $testPassed = isset($responseData['data']) || isset($responseData['models']);
                break;
            case 'gemini':
                $testPassed = isset($responseData['models']);
                break;
            case 'claude':
                $testPassed = isset($responseData['models']);
                break;
    ...
    error_log("Error en test_ai_provider_safe.php: " . $e->getMessage());
