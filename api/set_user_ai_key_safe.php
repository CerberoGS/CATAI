...
try {
    ...
    $stmt = $pdo->prepare('SELECT id, slug, name FROM ai_providers WHERE id = ? AND is_enabled = 1');
    $stmt->execute([$providerId]);
    $provider = $stmt->fetch(PDO::FETCH_ASSOC);
    
    if (!$provider) {
        json_out(['error' => 'provider-not-found', 'message' => 'Proveedor de IA no encontrado o deshabilitado'], 404);
    }
    ...
    error_log("Error en set_user_ai_key_safe.php: " . $e->getMessage());
