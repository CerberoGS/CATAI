    ...
    const optionsHtml = (() => {
      ...
      function render(exp, win){
        ...
        const rows = windowStrikes.map(s => {
          ...
          const trClass = s === atmStrike ? 'bg-indigo-50 dark-mode:bg-indigo-900/30' : '';
          return `
            <tr class=\"${trClass} border-b last:border-0 border-gray-200 dark-mode:border-gray-700\">
              <td class=\"px-2 py-1 text-xs md:text-sm\">${Config.fmt(c.last)}</td>
              <td class=\"px-2 py-1 text-xs md:text-sm\">${Config.fmt(c.bid)}</td>
              <td class=\"px-2 py-1 text-xs md:text-sm\">${Config.fmt(c.ask)}</td>
              <td class=\"px-2 py-1 text-xs md:text-sm\">${Config.fmt(c.iv)}</td>
              <td class=\"px-2 py-1 text-xs md:text-sm\">${Config.fmt(c.delta)}</td>
              <td class=\"px-2 py-1 text-xs md:text-sm font-medium text-center\">${Config.fmt(s)}</td>
              <td class=\"px-2 py-1 text-xs md:text-sm\">${Config.fmt(p.delta)}</td>
              <td class=\"px-2 py-1 text-xs md:text-sm\">${Config.fmt(p.iv)}</td>
              <td class=\"px-2 py-1 text-xs md:text-sm\">${Config.fmt(p.bid)}</td>
              <td class=\"px-2 py-1 text-xs md:text-sm\">${Config.fmt(p.ask)}</td>
              <td class=\"px-2 py-1 text-xs md:text-sm\">${Config.fmt(p.last)}</td>
            </tr>`;
        }).join('');
        ...
        return `
          ...
          <div class=\"overflow-x-auto rounded-lg border border-gray-200 dark-mode:border-gray-700\">
            <table class=\"min-w-full text-left text-xs md:text-sm\">
              <thead class=\"bg-gray-50 dark-mode:bg-gray-800\">
                <tr>
                  <th class=\"px-2 py-2\" colspan=\"5\">CALLS</th>
                  <th class=\"px-2 py-2 text-center\">Strike</th>
                  <th class=\"px-2 py-2\" colspan=\"5\">PUTS</th>
                </tr>
                <tr class=\"text-[11px] md:text-xs text-gray-600 dark-mode:text-gray-300\">
                  <th class=\"px-2 py-1\">Last</th>
                  <th class=\"px-2 py-1\">Bid</th>
                  <th class=\"px-2 py-1\">Ask</th>
