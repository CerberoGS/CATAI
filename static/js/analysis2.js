...
async function analyze() {
  ...
  try {
    ...
    const optionsHtml = (() => {
      ...
      if (optionsNote) {
        const exps = chain.chain.slice(0, 10).map(c => `<li class="text-sm">${c.expiration || '—'} · IV ${Config.fmt(c.iv)}</li>`).join('');
        return `
          <div class="text-sm">
            <p class="mb-2">En tu plan no hay strikes; expiraciones con IV promedio:</p>
            <ul class="list-disc ml-5 space-y-1">${exps}</ul>
          </div>`;
      }
      const call = callPick ? `<div class="p-3 rounded bg-green-50 dark-mode:bg-green-900/40 border border-green-200 dark-mode:border-green-700"><div class="text-xs text-gray-600 dark-mode:text-gray-300 mb-1">CALL ATM aproximado</div><div class="text-sm">Strike ${Config.fmt(callPick.strike)} · Bid ${Config.fmt(callPick.bid)} · Ask ${Config.fmt(callPick.ask)} · IV ${Config.fmt(callPick.iv)} · Δ ${Config.fmt(callPick.delta)}</div></div>` : '';
      const put  = putPick  ? `<div class="p-3 rounded bg-red-50 dark-mode:bg-red-900/40 border border-red-200 dark-mode:border-red-700"><div class="text-xs text-gray-600 dark-mode:text-gray-300 mb-1">PUT ATM aproximado</div><div class="text-sm">Strike ${Config.fmt(putPick.strike)} · Bid ${Config.fmt(putPick.bid)} · Ask ${Config.fmt(putPick.ask)} · IV ${Config.fmt(putPick.iv)} · Δ ${Config.fmt(putPick.delta)}</div></div>` : '';
      return `<div class="grid md:grid-cols-2 gap-3">${call}${put}</div>`;
    })();
