...
class AIBehavioralDashboard {
    ...
    async runBehavioralAnalysis(symbol) {
        ...
        // Usar el endpoint híbrido optimizado para análisis comportamental
        const response = await fetch(`${this.apiBase}/ai_analyze_hybrid_optimized.php`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${this.getToken()}`
        ...
        // Guardar análisis en historial si es exitoso
        if (result.text) {
            await this.saveAnalysisToHistory(symbol, result, analysisType);
        }
...
Generado por CATAI - Sistema de IA Comportamental`;
    ...
    // Función para guardar análisis en historial
    async saveAnalysisToHistory(symbol, result, analysisType) {
        try {
            const response = await fetch(`${this.apiBase}/ai_analysis_save_safe.php`, {
                method: 'POST',
                headers: {
            ...
            if (response.ok) {
                console.log('Análisis guardado en historial exitosamente');
            }
        } catch (error) {
