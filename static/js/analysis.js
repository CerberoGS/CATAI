  ...
  try {
    ...
    const optionsHtml = (() => {
      ...
      function render(exp, win){
        ...
        const hasOI = byExp.some(x => x.oi != null || x.openInterest != null);
        const hasVol = byExp.some(x => x.vol != null || x.volume != null || x.tradeVolume != null);
        ...
        const rows = windowStrikes.map(s => {
          ...
          const cOI = val(c, ['oi','openInterest']);
          const cVol = val(c, ['vol','volume','tradeVolume']);
          const pOI = val(p, ['oi','openInterest']);
          const pVol = val(p, ['vol','volume','tradeVolume']);
          ...
          return `
            <tr class=\"${trClass} border-b last:border-0 border-gray-200 dark-mode:border-gray-700\">
              ...
              ${hasVol ? `<td class=\"px-2 py-1 text-[11px] md:text-sm ${cellHLCall} ${itmCallBg}\" title=\"Volumen\">${Config.fmt(cVol)}</td>` : ''}
              ${hasOI  ? `<td class=\"px-2 py-1 text-[11px] md:text-sm ${cellHLCall} ${itmCallBg}\" title=\"Open Interest\">${Config.fmt(cOI)}</td>` : ''}
              ...
              ${hasOI  ? `<td class=\"px-2 py-1 text-[11px] md:text-sm ${cellHLPut} ${itmPutBg}\" title=\"Open Interest\">${Config.fmt(pOI)}</td>` : ''}
              ${hasVol ? `<td class=\"px-2 py-1 text-[11px] md:text-sm ${cellHLPut} ${itmPutBg}\" title=\"Volumen\">${Config.fmt(pVol)}</td>` : ''}
            </tr>`;
        }).join('');
        ...
        return `
          ...
          <div class=\"overflow-x-auto rounded-lg border border-gray-200 dark-mode:border-gray-700\">
            <table class=\"min-w-full text-left text-xs md:text-sm\">
              <thead class=\"bg-gray-50 dark-mode:bg-gray-800\" title=\"ITM/OTM/ATM según moneyness: Call ITM si strike < spot; Put ITM si strike > spot; ATM ≈ spot\">
                <tr>
                  <th class=\"px-2 py-2\" colspan=\"${5 + (hasVol?1:0) + (hasOI?1:0)}\" title=\"Lado Calls\">CALLS</th>
                  <th class=\"px-2 py-2 text-center\" title=\"Precio de ejercicio (strike)\">Strike</th>
                  <th class=\"px-2 py-2\" colspan=\"${5 + (hasOI?1:0) + (hasVol?1:0)}\" title=\"Lado Puts\">PUTS</th>
                </tr>
                <tr class=\"text-[11px] md:text-xs text-gray-600 dark-mode:text-gray-300\">
                  ...
                  ${hasVol ? '<th class=\\"px-2 py-1\\" title=\\"Volumen\\">Vol</th>' : ''}
                  ${hasOI  ? '<th class=\\"px-2 py-1\\" title=\\"Open Interest\\">OI</th>' : ''}
          ...
          <div class=\"mt-2 text-sm\">
            <div class=\"flex flex-wrap items-center gap-2\">
              ...
              <button id=\"oc-copy\" class=\"px-3 py-1 rounded bg-indigo-600 text-white hover:bg-indigo-700\">Copiar setup</button>
            </div>
          </div>`;
      }
      ...
      setTimeout(() => {
        ...
        const refresh = () => {
          ...
          setTimeout(() => {
            ...
            // Copiar setup
            document.getElementById('oc-copy')?.addEventListener('click', async () => {
              try {
                const expSel = document.getElementById('oc-exp')?.value || '';
                const stratSel = document.getElementById('oc-strategy')?.value || '';
                const priceSel = document.getElementById('oc-price')?.value || 'mid';
                const legsText = (function(){
                  try { return [ ...document.querySelectorAll('#oc-container table tbody tr') ]
                    .filter(tr => tr.querySelector('.ring-2'))
                    .map(tr => {
                      const tds = tr.querySelectorAll('td');
                      const strike = tds[5]?.textContent?.trim() || '';
                      // Detect leg side/type via highlight class presence
                      const isCall = tds[0]?.className?.includes('ring-2');
                      const isPut  = tds[tds.length-1]?.className?.includes('ring-2');
                      const side = 'BUY/SELL';
                      const type = isCall ? 'CALL' : (isPut ? 'PUT' : '');
                      return `${type} ${strike}`;
                    }).join(' | ');
                  } catch { return ''; }
                })();
                const text = `SETUP ${symbol} ${expSel} ${stratSel.replaceAll('_',' ').toUpperCase()}\nPrecio(${priceSel}): ${document.querySelector('#oc-container .inline-flex strong')?.textContent || entryText}\nTP: ${tpText} | SL: ${slText}\nLEGS: ${legsText}`;
                await navigator.clipboard.writeText(text);
                Config.toast('Setup copiado al portapapeles');
              } catch { Config.toast('No se pudo copiar setup'); }
            });
          });
        };
        document.getElementById('oc-exp')?.addEventListener('change', refresh);
...
// Construye un resumen de setup desde la option chain renderizada
function buildSetupSummaryFromDOM() {
  try {
    const container = document.getElementById('oc-container');
    if (!container) return '';
    const sym = (document.getElementById('symbol')?.value || document.getElementById('symbol-select')?.value || '').toUpperCase();
    const exp = container.querySelector('#oc-exp')?.value || '';
    const strat = container.querySelector('#oc-strategy')?.value || '';
    const priceMode = container.querySelector('#oc-price')?.value || 'mid';
    const price = container.querySelector('.inline-flex strong')?.textContent || '';
    ...
    container.querySelectorAll('table tbody tr').forEach(tr => {
      const tds = tr.querySelectorAll('td');
      const strike = (tds[5]?.textContent||'').trim();
      const callHL = tds[0]?.className?.includes('ring-2');
      const putHL  = tds[tds.length-1]?.className?.includes('ring-2');
      if (callHL) legs.push(`CALL ${strike}`);
      if (putHL)  legs.push(`PUT ${strike}`);
    });
    const legsStr = legs.join(' | ');
    return `SETUP ${sym} ${exp} ${strat.replaceAll('_',' ').toUpperCase()}\nPrecio(${priceMode}): ${price}\nTP: ${tp} | SL: ${sl}\nLEGS: ${legsStr}`;
  } catch { return ''; }
}
...
msOpen?.addEventListener('click', ()=>{ 
  ...
  const setupSummary = buildSetupSummaryFromDOM();
  ...
  if (msBody) {
    msBody.innerHTML = `
      ...
      <div>
        ...
        <textarea id="ms-notes" rows="4" class="mt-1 w-full rounded-md border-gray-300 shadow-sm focus:ring-indigo-500 focus:border-indigo-500" placeholder="Entrada, gestión, salida, aprendizajes...">${setupSummary ? (setupSummary + '\n\n') : ''}</textarea>
      </div>
      <div>
